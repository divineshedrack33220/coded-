...
const { createProfile, updateProfile, getProfile, getUserProfile, getNearbyUsers, getCurrentUser, getAccountDetails, upload, uploadImages } = require('../controllers/UserController');
...
router.post('/:id/rate', auth, async (req, res) => {
  try {
    ...
    const existingRating = user.rating.ratings.find(r => r.user.toString() === req.user._id.toString());
    if (existingRating) {
    ...
    user.rating.average = total / user.rating.count;
    await user.save();
    res.json({ message: 'Rating submitted', averageRating: user.rating.average });
  } catch (error) {
